...
const UserCompaniesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info userCompanies created successfully" });
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="userCompanies-create-dialog-component">
            <div>
            ...
            <div>
                <p className="m-0">Compan Id:</p>
                <InputText className="w-full mb-3" value={_entity?.companId} onChange={(e) => setValByKey("companId", e.target.value)}  />
            </div>
            <div>
                <p className="m-0">Effective Date:</p>
                <InputText className="w-full mb-3" value={_entity?.effectiveDate} onChange={(e) => setValByKey("effectiveDate", e.target.value)}  />
            </div>
            <div>
