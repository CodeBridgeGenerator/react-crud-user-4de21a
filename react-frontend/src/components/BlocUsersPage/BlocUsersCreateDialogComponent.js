...
const BlocUsersCreateDialogComponent = (props) => {
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="blocUsers-create-dialog-component">
            <div>
                <p className="m-0">Type:</p>
                <InputText className="w-full mb-3" value={_entity?.type} onChange={(e) => setValByKey("type", e.target.value)}  />
            </div>
            <div>
                <p className="m-0">Pattern:</p>
                <InputText className="w-full mb-3" value={_entity?.pattern} onChange={(e) => setValByKey("pattern", e.target.value)}  />
            </div>
            <div>
                <p className="m-0">Detail:</p>
                <InputText className="w-full mb-3" value={_entity?.detail} onChange={(e) => setValByKey("detail", e.target.value)}  />
            </div>
